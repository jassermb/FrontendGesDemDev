...
const Consulterddd = () => {
    ...
    const [cooAppreciation, setCOOAppreciation] = useState('');
    ...
    // Fonction pour détecter le scroll et afficher ou masquer le bouton
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const responseCOO = await axios.get(`http://localhost:5000/api/demdev/appreciationCOOC/${id}`);

                if (responseCOO.data && responseCOO.data.length > 0 && responseCOO.data[0].appreciationCOO) {
                    setCOOAppreciation(responseCOO.data[0].appreciationCOO);
                } 
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        const statut = showRejectionReason ? 'Rejetée par le COO' : 'Clôturée (COO)';
            ...
            >
                ...
                <Card style={{ border: 'none', boxShadow: 'none' }}>
                    ...
                    <div className="development-request-details-container">
                        ...
                        {/* Partie pour l'appréciation du COO */}
                        {cooAppreciation && (
                            <Paper elevation={0} sx={{ p: 3, marginTop: 2, backgroundColor: '#f0f4f7' }}>
                                <Typography variant="h6" gutterBottom sx={{
                                    color: '#3f51b5', borderBottom: '1px solid #243242', paddingBottom: '5px', marginBottom: '10px', fontSize: '1rem',
                                }}>
                                    Clôture par le COO :
                                ...
                                <Grid container spacing={3}>
                                    <Grid item xs={12}>
                                        <Box sx={{ p: 1.5, backgroundColor: '#fff', borderRadius: 1, border: 1, borderColor: 'grey.300' }}>
                                            <Typography variant="subtitle2" color="textSecondary">Appréciation de Clôture</Typography>
                                            <Typography variant="body2">{loading ? "Chargement..." : cooAppreciation}</Typography>
                                        </Box>
                                    </Grid>
                                </Grid>
                            </Paper>
